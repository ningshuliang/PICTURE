...
class TestDataset(Dataset):
    def __init__(self, path1,path2,path3, path4, path5, image_key="image", caption_key="txt"):
        ...
        self.parsing_files = os.listdir(self.parsing_path)
        self.parsing_files.sort()
        self.parsing_files = self.parsing_files[:400]
    ...
    def __getitem__(self, idx):
        image = self.tform(Image.open(self.image_path+'/'+self.parsing_files[idx]).convert("RGB"))
        image = image * 2. - 1.

        image_wo_cloth = self.tform(Image.open(self.image_wo_cloth_path+'/'+self.parsing_files[idx]).convert("RGB"))
        image_wo_cloth = image_wo_cloth * 2. - 1.

        parsing = self.tform(Image.open(self.parsing_path+'/'+self.parsing_files[idx]).convert("RGB"))
        parsing = parsing * 2. - 1.
        parsing_origin = np.array(Image.open(self.parsing_original_path+'/'+self.parsing_files[idx]))


        cloth = self.tform(Image.open(self.cloth_patch_path+'/'+self.parsing_files[idx]).convert("RGB"))
        cloth = cloth * 2. - 1.
        file_name = self.parsing_files[idx]

        parsing_image = np.array(Image.open(self.parsing_path+'/'+self.parsing_files[idx]))
